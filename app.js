...
async function clasificarImagen() {
    ...
    try {
        ...
        const prediccion = await modelo.classify(imagen);
        const top = prediccion[0];
        const etiqueta = top.className.split(",")[0].trim();
        ...
        if (mejorPerro > mejorGato && mejorPerro > 0.2) {
            resultado.textContent = "Es un perro";
            return;
        }

        if (mejorGato > mejorPerro && mejorGato > 0.2) {
            resultado.textContent = "Es un gato";
            return;
        }

        if (mejorGato === 0 && mejorPerro === 0) {
            resultado.textContent = "Objeto desconocido";
            return;
        }

        resultado.textContent = obtenerDecision(etiqueta);
    } catch (error) {
