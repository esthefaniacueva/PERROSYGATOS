...
function obtenerDecision(etiqueta) {
    ...
    return "No es un perro, ni un gato";
}
...
async function clasificarImagen() {
    ...
    try {
        ...
        if (mejorPerro > 0.35 && mejorPerro > mejorGato) {
            resultado.textContent = "Es un perro";
            return;
        }

        if (mejorGato > 0.35 && mejorGato > mejorPerro) {
            resultado.textContent = "Es un gato";
            return;
        }

        resultado.textContent = "No es un perro, ni un gato";
    } catch (error) {
